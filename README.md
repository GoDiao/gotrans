...
设置页会按这台机器给每个模型标出体积、预计占内存的比例，以及一个档位（合适／偏紧／吃力），磁盘空间不够时会拦下下载。内存那一侧是估算，只做提示，不阻止你下载任何模型。

| 模型 | 体积 | 说明 |
| --- | ---: | --- |
| Qwen3.5 9B (4-bit) | 约 6.0 GB | 通用模型，体积最大 |
| Gemma 4 E4B (4-bit) | 约 5.2 GB | 通用模型，文本处理功能也依赖它 |
| Gemma 4 E2B (4-bit) | 约 3.6 GB | 更小的通用模型 |
| Qwen3.5 4B (4-bit) | 约 3.1 GB | 通用模型，比 Gemma 4 E2B 更小 |
| Hy-MT2 1.8B (8-bit) | 约 1.9 GB | 翻译专用 |
| Hy-MT2 1.8B (4-bit) | 约 1.0 GB | 翻译专用 |
| Hy-MT2 1.8B (2-bit) | 约 601 MB | 翻译专用，策展 GGUF |
