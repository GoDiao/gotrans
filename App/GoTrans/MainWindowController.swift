...
/// 主窗口的 AppKit 宿主。GoTrans 仍是菜单栏 app；这里只负责一个当前 Space 的
...
final class MainWindowController: NSObject, NSToolbarDelegate {
    ...
    private let logger = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "com.godiao.GoTrans",
        category: "Windowing"
    )
    ...
    private func makeToolbar() -> NSToolbar {
        let toolbar = NSToolbar(identifier: .goTransMain)
...
private extension NSToolbar.Identifier {
    static let goTransMain = NSToolbar.Identifier("com.godiao.main-toolbar")
}

private extension NSToolbarItem.Identifier {
    static let engineStatus = NSToolbarItem.Identifier("com.godiao.engine-status")
    static let settings = NSToolbarItem.Identifier("com.godiao.settings")
}
