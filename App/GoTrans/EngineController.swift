...
final class EngineController {
    ...
    func start() {
        ...
        loadTask = Task {
            // 单实例守卫（验明正身）：仅真正的 GoTrans 实例才放弃启动，防双模型加载；
            // 无关 HTTP 服务占用端口不影响引擎，仅 API 启动时自然失败
            if await Self.isGoTransServing(settings.port) {
                guard generation == loadGeneration else { return }
                engineStatus = .failed("端口 \(settings.port) 已有 GoTrans 实例在运行")
                GTLog.error("startup aborted: another GoTrans on \(settings.port)")
                return
            }
    ...
    private static func isGoTransServing(_ port: UInt16) async -> Bool {
        ...
        return json["service"] as? String == "gotrans"
    }
}
